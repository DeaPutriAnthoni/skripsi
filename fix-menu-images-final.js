...
function updateMenuItem() {
  if (completed >= menuUpdates.length) {
    ...
    console.log('🔄 Please refresh browser (Ctrl+F5)');
    return;
  }
  
  const menu = menuUpdates[completed];
  
  // Create fresh connection for each update
  ...
  conn.connect((err) => {
    if (err) {
      console.log(`❌ Connection error for ${menu.name}:`, err.message);
      completed++;
    ...
    conn.query(
      ...
      (err, result) => {
        ...
        conn.end((err) => {
          completed++;
          setTimeout(updateMenuItem, 500); // Wait 500ms before next
        });
      }
    );
  });
